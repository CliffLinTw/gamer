...
void (*Par_Init_ByFunction_AfterAcceleration_Trace_Ptr)( const long NPar_ThisRank, const long NPar_AllRank,
                                 real *ParMass, real *ParPosX, real *ParPosY, real *ParPosZ,
                                 real *ParVelX, real *ParVelY, real *ParVelZ,
                                 real *ParAccX, real *ParAccY, real *ParAccZ,
...
/*
void AddNewParticleAttribute_TraceParticle()
{
   if ( Idx_ParLabel == Idx_Undefined )
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Par_Init_ByFunction_TraceParticle
...
void Par_Init_ByFunction_TraceParticle(const long NPar_ThisRank, const long NPar_AllRank, real *ParMass, real *ParPosX,
...
//      mass
        Mass_AllRank[p] = ParM;
//      id
//        AllAttribute[Idx_ParLabel][p] = p;

//      position
        Ran  = RNG->GetValue( 0, 0.0, 1.0);
        RanR = Ran*Disc_Radius;
        RanV = 0;
        RanVec3_FixRadius( RanR, RanVec, NormVec, RanV );
	//printf("%f\n", RanR);
	//printf("%f\n", RanVec[0]);
	//printf("%f\n", RanVec[1]);
	//printf("%f\n", RanVec[2]);
...
void Par_Init_ByFunction_AfterAcceleration_Trace(const long NPar_ThisRank, const long NPar_AllRank, real *ParMass,
...
// ============================================================================================================
   ...
   //   initialize the RNG
   RNG = new RandomNumber_t( 1 );
   RNG->SetSeed( 0, Disc_RSeed+10000 );

   double ZeroR = 0;
   double ZeroRVec[3], VD_Vec[3], V_acc_Vec[3];
   ...
   for (long p=0; p<NPar_ThisRank; p++)
   {
      ...
      ParRadius[2] =  ParPos[2][p]-Soliton_Cen[2];
      NormParRadius[0] =   ParRadius[0]/ sqrt( SQR(ParRadius[0]) + SQR(ParRadius[1])+ SQR(ParRadius[2]) );
      NormParRadius[1] =   ParRadius[1]/ sqrt( SQR(ParRadius[0]) + SQR(ParRadius[1])+ SQR(ParRadius[2]) );
      NormParRadius[2] =   ParRadius[2]/ sqrt( SQR(ParRadius[0]) + SQR(ParRadius[1])+ SQR(ParRadius[2]) );

      V_acc = sqrt(fabs(ParRadius[0]*ParAcc[0][p]+ParRadius[1]*ParAcc[1][p]+ParRadius[2]*ParAcc[2][p]));

      RanVec3_FixRadius( ZeroR, ZeroRVec, VD_Vec, VelDisp*V_acc );
      RanVec3_FixRadius( ZeroR, ZeroRVec, V_acc_Vec, V_acc );

      ParVel[0][p] =   V_acc_Vec[0]+Soliton_BulkVel[0]+VD_Vec[0];
      ParVel[1][p] =   V_acc_Vec[1]+Soliton_BulkVel[1]+VD_Vec[1];
      ParVel[2][p] =   V_acc_Vec[2]+Soliton_BulkVel[2]+VD_Vec[2];
   }
// ============================================================================================================
...
void RanVec3_FixRadius( const double r, double RanVec[], double NormVec[], const double RanV )
{
   double d1,d2,d3,dd1,dd2,dd3,NormL;
   d1 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );
   d2 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );  // not spherical, but cubic
   d3 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );

   RanVec[0] = r*d1/sqrt(d1*d1+d2*d2+d3*d3);
   RanVec[1] = r*d2/sqrt(d1*d1+d2*d2+d3*d3);
   RanVec[2] = r*d3/sqrt(d1*d1+d2*d2+d3*d3);

   dd1 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );
   dd2 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );
   dd3 = 1.0*RNG->GetValue( 0, 0.0, 1.0 );

   NormVec[0] = dd1 - d1*(d1*dd1+d2*dd2+d3*dd3)/(d1*d1+d2*d2+d3*d3);
   NormVec[1] = dd2 - d2*(d1*dd1+d2*dd2+d3*dd3)/(d1*d1+d2*d2+d3*d3);
   NormVec[2] = dd3 - d3*(d1*dd1+d2*dd2+d3*dd3)/(d1*d1+d2*d2+d3*d3);

   NormL = sqrt(NormVec[0]*NormVec[0]+NormVec[1]*NormVec[1]+NormVec[2]*NormVec[2]);

   NormVec[0] = RanV*NormVec[0]/NormL;
   NormVec[1] = RanV*NormVec[1]/NormL;
   NormVec[2] = RanV*NormVec[2]/NormL;
}

//-------------------------------------------------------------------------------------------------------
// Function    :  BC_TraceParticle
...
#if ( MODEL == ELBDM && defined GRAVITY )
void BC_TraceParticle( real fluid[], const double x, const double y, const double z, const double Time,
         const int lv, double AuxArray[] )
{
...
//-------------------------------------------------------------------------------------------------------
//// Function    :  Init_TestProb_ELBDM_TraceParticle
...
#  endif
#  ifdef PARTICLE
   Par_Init_ByFunction_Ptr     = Par_Init_ByFunction_TraceParticle;         // option: PAR_INIT=1;            example: Particle/Par_Init_ByFunction.cpp
   Par_Init_Attribute_User_Ptr = NULL;
//   Par_Init_Attribute_User_Ptr = AddNewParticleAttribute_TraceParticle;    // set PAR_NATT_USER;             example: TestProblem/Hydro/AGORA_IsolatedGalaxy/Init_TestProb_Hydro_AGORA_IsolatedGalaxy.cpp --> AddNewParticleAttribute()
   Init_User_Ptr               = Init_User_TraceParticle;
#  endif
